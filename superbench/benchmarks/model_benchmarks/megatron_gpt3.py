...
class MegatronGPT(ModelBenchmark):
    ...
    def add_parser_arguments(self):
        ...
        self._parser.add_argument(
            "--code_base", type=str, required=False, default="", help="Code base."
        )
        self._parser.add_argument(
            "--dataset_url", type=str, required=False, default=None, help="Dataset URL."
        )
        self._parser.add_argument(
            "--vocab_url",
            type=str,
        ...
        self._parser.add_argument(
            "--model_size", type=int, required=False, default=6.7, help="Model size."
        )
        self._parser.add_argument(
            "--num_layers",
            type=int,
        ...
        self._parser.add_argument(
            "--hidden_size", type=int, required=False, default=4096, help="Hidden size."
        )
        self._parser.add_argument(
            "--num_attn_heads",
            type=int,
        ...
        self._parser.add_argument(
            "--lr", type=float, required=False, default=1.2e-4, help="Learning rate."
        )
        self._parser.add_argument(
            "--min_lr",
            type=float,
        ...
        self._parser.add_argument(
            "--init_std", type=float, required=False, default=0.009, help="Init std."
        )
        self._parser.add_argument(
            "--seq_len", type=int, required=False, default=2048, help="Sequence length."
        )
        self._parser.add_argument(
        ...
        self._parser.add_argument(
            ...
            required=False,
            default=8,
            help="Number of GPUs per node to run the benchmark.",
        )
        self._parser.add_argument(
            "--num_nodes",
            type=int,
            required=False,
            default=1,
            help="Number of nodes to run the benchmark.",
        )
        self._parser.add_argument(
        ...
        self._parser.add_argument(
            ...
            action="store_true",
            help="No gradient accumulation fusion.",
        )
        self._parser.add_argument(
        ...
        self._parser.add_argument(
            ...
            action="store_true",
            help="No masked softmax fusion.",
        )
        self._parser.add_argument(
            "--no_bias_gelu_fusion", action="store_true", help="No bias gelu fusion."
        )
        self._parser.add_argument(
            "--no_bias_dropout_fusion",
            action="store_true",
            help="No bias dropout fusion.",
        )
        self._parser.add_argument(
            "--train_tokens",
            type=int,
        ...
        # Parallelism configs
        self._parser.add_argument(
            "--zero_stage", type=int, default=1, help="Zero stage."
        )
        # Misc configs
        self._parser.add_argument(
            "--log-interval", type=int, required=False, default=1, help="Log interval."
        )
        self._parser.add_argument(
            "--eval_iters", type=int, default=0, help="Eval iters."
        )
        self._parser.add_argument(
            "--eval_interval", type=int, default=10, help="Eval interval."
        )
        self._parser.add_argument(
            "--num_save", type=int, default=10000, help="Num save."
        )
        self._parser.add_argument(
            "--save_interval", type=int, default=10000, help="Save interval."
        )
        # Output and data configs
        self._parser.add_argument("--seed", type=int, default=1234, help="Seed.")
        self._parser.add_argument(
            "--data_home", type=str, default="/tmp", help="Data home."
        )
        self._parser.add_argument(
            "--vocab_path", type=str, default="/tmp/gpt2-vocab.json", help="Vocab path."
        )
        self._parser.add_argument(
            "--merge_path", type=str, default="/tmp/gpt2-merges.txt", help="Merge path."
        )
        self._parser.add_argument(
            "--split",
            type=str,
        ...
        self._parser.add_argument(
            "--prescale_grad", action="store_true", help="Prescale grad."
        )
        self._parser.add_argument(
            "--hostfile",
            type=str,
            default=None,
            help="Hostfile to run the mutli-node benchmark.",
        )
        self._parser.add_argument(
            "--data_impl", type=str, default="mmap", help="Data impl."
        )
        self._parser.add_argument(
            "--data_prefix",
            type=str,
            default="dataset_text_document",
            help="Data prefix.",
        )
        self._parser.add_argument(
        ...
        self._parser.add_argument(
            "--extra", type=str, default="", help="Extra options."
        )
        self._parser.add_argument(
            "--mock_data", action="store_true", help="Use mock data."
        )
        self._parser.add_argument(
            "--model",
            type=str,
        ...
        self._parser.add_argument(
            ...
            type=str,
            default=None,
            help="Transformer implementation.",
        )
    ...
    def _preprocess(self):
        ...
        data_parallel_size = (
            self._args.num_gpus
            * self._num_nodes
            // self._args.pipeline_model_parallel_size
            // self._args.tensor_model_parallel_size
        )
        if self._args.micro_batch_size < 1 or self._args.micro_batch_size > (
            self._args.batch_size // data_parallel_size
        ):
            logger.error(
                "Micro Batch size * data parallel size is larger than global batch size."
            )
            self._result.set_return_code(ReturnCode.INVALID_ARGUMENT)
            return False
    ...
    def _parse_log(self, output):
        ...
        elapsed_time_pattern = re.compile(
            r"elapsed time per iteration \(ms\): (\d+\.\d+)"
        )
            ...
            if "max allocated" in line:
                ...
                if max_mem_allocated_match:
                    max_mem_allocated_value = (
                        float(max_mem_allocated_match.group(1)) / 1024
                    )
    ...
    def __prepare_deespeed_config(self, precision_megatron):
        ...
        self._config_json_path = os.path.join(
            self._args.data_home, "ds_config_gpt.json"
        )
        ...
        ds_config_template = {
            ...
            "zero_optimization": {"stage": self._args.zero_stage},
    ...
    def _megatron_command(self, precision):  # noqa: C901
        ...
        if self._args.no_async_tensor_model_parallel_allreduce:
            megatron_options = (
                f"{megatron_options} --no-async-tensor-model-parallel-allreduce"
            )
        ...
        if self._args.extra and len(self._args.extra) > 0:
            ...
            print(
                f"Extra options: {self._args.extra}, {type(self._args.extra)}"
            )
            megatron_options = f"{megatron_options} {self._args.extra}"
        if self._args.expert_model_parallel_size:
            megatron_options = f"{megatron_options} --expert-model-parallel-size {self._args.expert_model_parallel_size}"
        if self._args.num_experts:
            megatron_options = f"{megatron_options} --num-experts {self._args.num_experts}"
        if self._args.max_padding_length:
            megatron_options = f"{megatron_options} --max-padding-length {self._args.max_padding_length}"
        if self._args.transformer_impl:
            megatron_options = f"{megatron_options} --transformer-impl {self._args.transformer_impl}"

        command = ""
        script_path = os.path.join(
            self._args.code_base, f"pretrain_{self._args.model}.py"
        )
        if self._args.deepspeed:
            deepspeed_option = self.__prepare_deespeed_config(
                precision_megatron.lstrip("--")
            )
            # No --log-throughput in Megatron-DeepSpeed by 20231219
            megatron_options = megatron_options.replace("--log-throughput", "").strip()
            if self._num_nodes > 1:
                command = (
                    f"torchrun {self._distributed_args} "
    ...
    def _train_step(self, precision):  # noqa: E501
        ...
        if (
            self._num_nodes == 1
            or int(os.environ["OMPI_COMM_WORLD_RANK"])
            == int(os.environ["OMPI_COMM_WORLD_SIZE"]) - 1
            or int(os.environ["OMPI_COMM_WORLD_RANK"]) == 0
        ):
            iteration_times, tflops, mem_allocated, max_mem_allocated = self._parse_log(
                output.stdout
            )
            if len(tflops) > 0:
                info["tflops"] = tflops
    ...
    def _init_distributed_setting(self):
        ...
        if not os.getenv("OMPI_COMM_WORLD_SIZE"):
            ...
            return False
        self._num_nodes = int(os.getenv("OMPI_COMM_WORLD_SIZE")) // int(
            os.getenv("OMPI_COMM_WORLD_LOCAL_SIZE")
        )
        master_addr = "localhost"
        if self._num_nodes > 1:
            if not self._args.hostfile:
                sb_hostfile = os.path.join(
                    os.environ.get("SB_WORKSPACE", "."), "hostfile"
                )
                if os.path.exists(sb_hostfile):
                    hosts = open(sb_hostfile).read().split("\n")
                    hosts = [
                        f"{host} slots={self._args.num_gpus}"
                        for host in hosts
                        if host != ""
                    ]
        ...
        node_rank = int(os.environ["OMPI_COMM_WORLD_RANK"]) // int(
            os.environ["OMPI_COMM_WORLD_LOCAL_SIZE"]
        )
        self._distributed_args = (
            f"--nproc_per_node {self._args.num_gpus} --nnodes {self._num_nodes} "
    ...
    def _generate_dataset(self):
        ...
        if self._args.mock_data:
            ...
            if not os.path.exists(
                os.path.join(self._args.data_home, f"{self._args.data_prefix}.bin")
            ) or not os.path.exists(
                os.path.join(self._args.data_home, f"{self._args.data_prefix}.idx")
            ):
                if self._args.dataset_url:
                    ...
                    if not os.path.exists(
                        os.path.join(
                            self._args.data_home, f"{self._args.data_prefix}.bin"
                        )
                    ) or not os.path.exists(
                        os.path.join(
                            self._args.data_home, f"{self._args.data_prefix}.idx"
                        )
                    ):
                        logger.error("Dataset failed to generate.")
                        self._result.set_return_code(
                            ReturnCode.DATASET_GENERATION_FAILURE
                        )
                        return False
            ...
            self._data_path = os.path.join(
                self._args.data_home, f"{self._args.data_prefix}"
            )
...
BenchmarkRegistry.register_benchmark(
    "megatron-gpt", MegatronGPT, parameters="", platform=Platform.CUDA
)
BenchmarkRegistry.register_benchmark(
    "megatron-gpt", MegatronGPT, parameters="", platform=Platform.ROCM
)
