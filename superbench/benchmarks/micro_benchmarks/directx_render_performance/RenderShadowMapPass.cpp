...
int RenderShadowMapPass::DefineStaticSamplers(std::vector<CD3DX12_STATIC_SAMPLER_DESC> &samplers)
{
	return 0;
}

void RenderShadowMapPass::CreateShaderResourceView(ID3D12Device *device, ID3D12GraphicsCommandList *cmdList, int width, int height)
{
	return;
}
...
void RenderShadowMapPass::SetStatesBeforeDraw(ID3D12GraphicsCommandList *cmdList)
{
	...
	cmdList->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0,
								   0, nullptr);
...
void RenderShadowMapPass::Draw(ID3D12GraphicsCommandList *cmdList)
{
