 ...
 */
bool InitMainWindow(HINSTANCE hInstance, int width, int height, HWND &hMainWnd, const std::wstring &winTitle, bool quiet_mode)
{
	WNDCLASS wc;
	...
	hMainWnd = CreateWindow(wc.lpszClassName, winTitle.c_str(), WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width,
							height, 0, 0, hInstance, 0);
	if (!hMainWnd)
	{
		return false;
	}
 ...
 */
std::unique_ptr<RenderApp> get_render_pointer(BenchmarkOptions &args, HINSTANCE hInstance, HWND hMainWnd, std::wstring &winTitle)
{
