...
void RenderLightingPass::CreateShaderResourceView(ID3D12Device *device, ID3D12GraphicsCommandList *cmdList, int width, int height)
{
...
int RenderLightingPass::DefineRootParameters(std::vector<CD3DX12_ROOT_PARAMETER> &rootParameters)
{
	...
	rootParameters[rootParametersIndex].InitAsConstantBufferView(rootParametersIndex, 0, D3D12_SHADER_VISIBILITY_VERTEX);
	rootParametersIndex++;
	...
	descriptorRange[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, m_numShaderResource, 0, 0); // Using valid D3D12_DESCRIPTOR_RANGE_TYPE_SRV
	rootParameters[rootParametersIndex].InitAsDescriptorTable(1, descriptorRange.release(), D3D12_SHADER_VISIBILITY_PIXEL);
	rootParametersIndex++;
...
void RenderLightingPass::BuildPipelineStates(ID3D12Device *device)
{
	...
	ComPtr<ID3DBlob> vertexShader = CompileShader(L"Shaders/DefferredLightingVertex.hlsl", nullptr, "RadialVertexMain", "vs_5_1");
	ComPtr<ID3DBlob> pixelShader = CompileShader(L"Shaders/DefferredLightingPixel.hlsl", nullptr, "DeferredLightPixelMain", "ps_5_1");
	...
	pixelShader = CompileShader(L"Shaders/DefferredLightingPixel.hlsl", nullptr, "MainOnePassPointLightShadowPS", "ps_5_1");
...

{
	...
	int samplerIndex = 0;
	CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_SceneDepthTextureSampler(samplerIndex,						// shaderRegister
	...
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_GBufferATextureSampler(samplerIndex,						// shaderRegister
																				 D3D12_FILTER_MIN_MAG_MIP_POINT,	// filter
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressU
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressV
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	samplerData[samplerIndex++] = SceneTexturesStruct_GBufferATextureSampler;
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_GBufferBTextureSampler(samplerIndex,						// shaderRegister
																				 D3D12_FILTER_MIN_MAG_MIP_POINT,	// filter
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressU
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressV
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	samplerData[samplerIndex++] = SceneTexturesStruct_GBufferBTextureSampler;
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_GBufferCTextureSampler(samplerIndex,						// shaderRegister
																				 D3D12_FILTER_MIN_MAG_MIP_POINT,	// filter
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressU
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressV
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	samplerData[samplerIndex++] = SceneTexturesStruct_GBufferCTextureSampler;
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_GBufferDTextureSampler(samplerIndex,						// shaderRegister
																				 D3D12_FILTER_MIN_MAG_MIP_POINT,	// filter
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressU
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressV
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	samplerData[samplerIndex++] = SceneTexturesStruct_GBufferDTextureSampler;
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_GBufferETextureSampler(samplerIndex,						// shaderRegister
																				 D3D12_FILTER_MIN_MAG_MIP_POINT,	// filter
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressU
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	// addressV
																				 D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	samplerData[samplerIndex++] = SceneTexturesStruct_GBufferETextureSampler;
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_ScreenSpaceAOTextureSampler(samplerIndex,						 // shaderRegister
																					  D3D12_FILTER_MIN_MAG_MIP_POINT,	 // filter
																					  D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	 // addressU
																					  D3D12_TEXTURE_ADDRESS_MODE_CLAMP,	 // addressV
																					  D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
	...
	const CD3DX12_STATIC_SAMPLER_DESC SceneTexturesStruct_CustomDepthTextureSampler(8,								   // shaderRegister
																					D3D12_FILTER_MIN_MAG_MIP_POINT,	   // filter
																					D3D12_TEXTURE_ADDRESS_MODE_CLAMP,  // addressU
																					D3D12_TEXTURE_ADDRESS_MODE_CLAMP,  // addressV
																					D3D12_TEXTURE_ADDRESS_MODE_CLAMP); // addressW
...
/* bool RenderLightingPass::Initialize()
{
	// Init D3D device, context and command objects.
	InitDirect3D();
	CreateSwapChain();
	...
	m_gpuTimer.init(m_device.Get(), m_maxTimerNum);
	m_outFile.open(m_opts->m_outfile, std::ios::trunc);

	return true;
} */

/* void RenderLightingPass::Runtest()
{
	// Initialize();
	LoadAssets();

	for (int i = 0; i < m_opts->m_num_frames; i++)
	{
		// m_gpuTimer.start(m_commandList.Get(), i);
		...
		// m_gpuTimer.stop(m_commandList.Get(
	}
...
int main(int argc, char **argv)
{
	...
	Options opts;
	// Create the renderer.
	RenderLightingPass renderer(opts);
	renderer.Initialize();
	renderer.LoadAssets();
	// Run the renderer.
	renderer.Runtest();
...
{
	Options args(GetCommandLineW());
	RenderLightingPass theApp(hInstance, args);
