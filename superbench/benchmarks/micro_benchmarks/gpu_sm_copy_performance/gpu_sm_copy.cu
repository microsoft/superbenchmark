...
void PrintUsage() { printf("Usage: gpu_sm_copy <gpu-id> <copy-direction: dtoh|htod> <size> <num_loops>\n"); }
...
int DestroyBuf() {
    ...
    if (g_data_buf != nullptr)
        free(g_data_buf);
    if (g_check_buf != nullptr)
        free(g_check_buf);
...
int BenchSMCopyKernel() {
    ...
    for (int i = 0; i < g_num_loops; i++) {
        SMCopyKernel<<<num_thread_blocks, NUM_THREADS_IN_BLOCK, 0, stream>>>(reinterpret_cast<ulong2 *>(g_tgt_buf),
