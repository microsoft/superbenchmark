...
+ `type`: the traffic pattern type, required.
    * accepted values: `all-nodes`, `k-batch`, `topo-aware`
+ `scale`: the scale of batch in `k-batch` pattern, required.
+ `ibstat`: the path of ibstat output in `topo-aware` pattern, wil be auto-generated in `./output/ibstat_file.txt` if not specified.
+ `ibnetdiscover`: the path of ibnetdiscover output in `topo-aware` pattern, required.
+ `min_dist`: minimum distance of VM pair in `topo-aware` pattern, required.
+ `max_dist`: maximum distance of VM pair in `topo-aware` pattern, required.
