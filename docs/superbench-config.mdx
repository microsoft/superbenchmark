...
Enable monitor to collect system metrics periodically, currently only support CUDA platform. There have three settings:
...
Calculate the average metirc during sample_duration seconds, such as CPU usage and NIC bandwidth.

#### `sample_freq`

Do sampling every sample_freq seconds.
