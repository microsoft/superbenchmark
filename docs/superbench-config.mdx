...
+ `batch`: the scale of batch in `k-batch` pattern, required.
+ `ibstat`: the path of ibstat output in `topo-aware` pattern, wil be auto-generated in `./output/ibstat_file.txt` if not specified.
+ `ibnetdiscover`: the path of ibnetdiscover output `ibnetdiscover_file.txt` in `topo-aware` pattern, required.
