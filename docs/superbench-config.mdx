...
+ `mpi-parallel`: launch benchmark through MPI, the benchmark implementation could leverage MPI interface, each MPI job could run in parallel.

Some attributes may only be suitable for specific mode.

|            | `local` | `torch.distributed` | `mpi` | `mpi-parallel` |
| ---------: | :-----: | :-----------------: | :---: | :-------------: | 
| `proc_num` |    ✓    |          ✓          |   ✓   |       ✓       |
| `node_num` |    ✘    |          ✓          |   ✓   |       ✓       |
| `prefix`   |    ✓    |          ✘          |   ✘   |       ✘       |
...
* accepted values: `local | torch.distributed | mpi | mpi-parallel`
* default value: `local`
...
Only available for `local` mode and `mpi-parallel` mode.
