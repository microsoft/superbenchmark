...
class TensorRTInferenceBenchmarkTestCase(unittest.TestCase):
    ...
    def test_tensorrt_inference_result_parsing(self, test_raw_log):
        ...
        for tag in ['mean', '99']:
            self.assertEqual(0.5, benchmark.result[f'gpu_time_{tag}'][0])
            self.assertEqual(0.6, benchmark.result[f'host_time_{tag}'][0])
            self.assertEqual(1.0, benchmark.result[f'end_to_end_time_{tag}'][0])
