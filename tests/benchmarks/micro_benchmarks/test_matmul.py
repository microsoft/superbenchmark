...
def test_pytorch_matmul():
    ...
    assert (len(benchmark.raw_data['nosharding_time_ms']) == benchmark.run_count)
    assert (len(benchmark.raw_data['nosharding_time_ms'][0]) == benchmark._args.num_steps)
    assert (len(benchmark.result['nosharding_time_ms']) == benchmark.run_count)
