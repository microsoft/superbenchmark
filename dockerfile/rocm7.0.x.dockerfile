...
#   - hipblaslt: release-staging/rocm-rel-7.0 (hipblaslt-bench only, against system hipBLASLt)
#   - rocblas: release-staging/rocm-rel-7.0
#   - transformer_engine: v2.4_rocm
#   - openmpi: 4.1.x
# Intel:
#   - mlc: v3.12
# Network:
...
RUN apt-get update && \
    make RCCL_HOME=/opt/rccl/build/ ROCBLAS_BRANCH=release-staging/rocm-rel-7.0 HIPBLASLT_BRANCH=release-staging/rocm-rel-7.0 ROCM_VER=rocm-5.5.0 -C third_party rocm -o cpu_hpl -o cpu_stream -o megatron_lm -o rocm_hipblaslt -o rocm_megatron_lm -o apex_rocm
...
# generation that the ROCm 6.4 image needs. hipBLASLt 7.0 notes:
#   - rocRoller is controlled by HIPBLASLT_USE_ROCROLLER (was HIPBLASLT_ENABLE_ROCROLLER); disable
#     it together with its mxDataGenerator client dependency.
#   - Skip the Ubuntu-only hipblaslt-sequence client, which requires LLVM dev packages.
#   - Drop the rocisa dependency of the (skipped) Tensile library target.
#   - Rename the deps project's reserved "install" target so it configures without CMP0037 OLD.
#   - Pre-build the googletest/lapack client deps normally built by `./install.sh -d`, since
#     `install.sh -dc` would also build the full library.
RUN cd third_party && \
    ...
    cmake .. && \
    cmake --build . -j${NUM_MAKE_JOBS} --target googletest lapack && \
    cmake --build gtest/src/googletest-build -j${NUM_MAKE_JOBS} --target install && \
    ...
    set -- cmake \
        -DAMDGPU_TARGETS="${hipblaslt_architectures}" \
...
# Install TransformerEngine - pin to v2.4_rocm, the ROCm 7.0 release line (it bundles the AOTriton
# 0.10b ROCm 7.0 runtime and supports gfx950). CK fused attention stays disabled, as in the
