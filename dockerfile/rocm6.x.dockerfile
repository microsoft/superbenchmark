ARG BASE_IMAGE=rocm/pytorch-training:v25.5
...
RUN UBUNTU_VERSION=$(awk -F'=' '/^VERSION_ID=/{gsub(/"/, "", $2); print $2}' /etc/os-release) && \
    if ! command -v ofed_info >/dev/null 2>&1; then \
    echo "Installing OFED for Ubuntu ${UBUNTU_VERSION}" && \
    cd /tmp && \
...
RUN make RCCL_HOME=/opt/rccl/build/ ROCBLAS_BRANCH=release-staging/rocm-rel-6.3 HIPBLASLT_BRANCH=release-staging/rocm-rel-6.3 ROCM_VER=rocm-5.5.0 -C third_party rocm -o cpu_hpl -o cpu_stream -o megatron_lm -o rocm_hipblaslt -o megatron_deepspeed
